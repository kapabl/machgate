/*
 * sgr_extract.c — Extract PUNKCAKE Sugar engine .sgr archives
 *
 * The SGR format:
 *   1. Raw file is XOR-encrypted with a seeded xoshiro128 PRNG
 *      - Seed: last 4 bytes of file, big-endian, XOR'd with 0x550f0f55
 *      - Skip rnd()&0xf random calls after seeding
 *      - XOR every other byte (stride 2) with (uint8_t)rnd()
 *   2. After decryption: 4-byte big-endian decompressed size + zlib data
 *   3. Decompressed data contains:
 *      - 2-byte title length + title string
 *      - 2-byte dev length + dev string
 *      - 2-byte root length + root string
 *      - Entries: 2-byte name length + name string + 1-byte type char +
 *        4-byte big-endian data size + XOR-encrypted data (seed in last 4 bytes)
 *
 * Build: cc -o sgr_extract sgr_extract.c -lz
 * Usage: sgr_extract data.sgr [output_dir]
 */
...
static void decode_buffer(uint8_t *buf, size_t len)
{
    ...
    /* Seed from last 4 bytes (big-endian) XOR'd with magic constant */
    ...
    /* XOR every other byte */
    for (size_t i = 0; i < data_len; i += 2)
        buf[i] ^= (uint8_t)sugar_rnd();
}

/* Encode filename characters that some filesystems (exfat/FAT/NTFS used on
 * handheld /userdata) reject (star, question-mark, colon, lt, gt, pipe,
 * dquote).  Directory separators are preserved.  sgr_repack reverses this
 * encoding.  Wildcard entries like assets.gfx.STAR.png occur in the archive
 * for Sugar engine directory-listing lookups and must be preserved. */
 ...
 * If a path component exists as a regular file (e.g., entry "code" was written
 * before "code/foo.lua"), rename it out of the way so the directory can be created. */
...
{
    /* Strip leading ../ to keep files inside output dir */
    while (strncmp(name, "../", 3) == 0)
        name += 3;

    /* Encode filesystem-unsafe characters (wildcards etc.) so exfat/FAT/NTFS
     * filesystems on handheld /userdata accept the filename. */
    char encoded[4096];
...
int main(int argc, char **argv)
{
    ...
    /* Parse entries: name_len(2) + name + type(1) + data_size(4) + data */
    int count = 0;
    while (pos + 7 < dest_len) {
        uint16_t name_len = read_be16(dec + pos); pos += 2;
        if (name_len == 0 || name_len > 2000 || pos + name_len + 5 > dest_len) break;

        char name[4096] = {0};
        memcpy(name, dec + pos, name_len < 4095 ? name_len : 4095);
        pos += name_len;

        uint8_t type_char = dec[pos]; pos += 1;
        ...
        printf("  %s (%u bytes, type='%c')\n", name, data_size,
               type_char >= 32 && type_char < 127 ? type_char : '?');
        fprintf(manifest, "ENTRY\t%d\t%s\n", type_char, name);

        /* Per-entry XOR decryption (same scheme as outer layer) */
        if (data_size > 4) {
            uint8_t *entry_data = malloc(data_size);
            memcpy(entry_data, dec + pos, data_size);
            decode_buffer(entry_data, data_size);
            /* After decryption, strip the 4-byte seed trailer */
