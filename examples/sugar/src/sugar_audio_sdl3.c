/*
 * sugar::audio::_load_ogg detour (current Sugar engine / SDL3 builds,
 * e.g. Shotgun King). Caches the decoded + resampled PCM via audio_cache.
 *
 * Signature: (std::string, sample_t*, bool, spec_t*) -> sample_t*
 *
 * The first-launch cache is populated at install time by patch.bash using
 * oggdec + pcm_cache_write against a shipped manifest, not at runtime.
 * This detour only handles cache hits and lazy fallback stores for any
 * file the manifest missed (e.g. DLC).
 */
	...
	int64_t, int64_t arg1, int64_t arg2, int64_t arg3, int64_t arg4)
{
	if (!call_original || !use_freq_ptr) {
		fprintf(stderr, "sugar_patches_sdl3: _load_ogg detour not initialized\n");
		return 0;
	}
	uint32_t use_freq = *use_freq_ptr;
	char key[33];
	if (audio_cache_probe(arg1, arg2, use_freq, "_load_ogg", key))
		return arg2;
	int64_t ret = call_original(arg1, arg2, arg3, arg4);
	if (ret == 0)
		return 0;
	audio_cache_store(arg1, arg2, use_freq, "_load_ogg", key);
	return ret;
}

__attribute__((constructor))
static void sugar_audio_sdl3_init(void)
{
	...
	if (!lr || !resolver_lookup_symbol || !lr->mh) {
		fprintf(stderr, "sugar_patches_sdl3: machismo internals unavailable\n");
		return;
	}
	...
	if (!use_freq_ptr) {
		fprintf(stderr, "sugar_patches_sdl3: __use_freq not found\n");
		return;
	}
	...
	fprintf(stderr,
	        "sugar_patches_sdl3: ready; _load_ogg=%p(->%p) __use_freq@%p=%u\n",
