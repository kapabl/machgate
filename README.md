...
- GitHub release: <https://github.com/kapabl/machgate/releases/tag/v0.3.9>
- Download: `machgate-0.3.9-linux-arm64.tar.gz`
- Latest validation:
...
```bash
MACHGATE_VERSION=0.3.9 scripts/run-macho-docker.sh /path/to/macos-arm64-binary
```
...
```bash
MACHGATE_TARBALL=/path/to/machgate-0.3.9-linux-arm64.tar.gz scripts/run-macho-docker.sh /path/to/macos-arm64-binary
```

To diagnose startup hangs in static constructors, enable LC_MAIN tracing:

```bash
MACHGATE_VERBOSE=1 MACHGATE_TRACE_LCMAIN=1 MACHGATE_TARBALL=/path/to/machgate-0.3.9-linux-arm64.tar.gz scripts/run-macho-docker.sh /path/to/macos-arm64-binary
```

To stop a stuck guest after a fixed interval while keeping live logs:

```bash
MACHGATE_TIMEOUT=120 MACHGATE_VERBOSE=1 MACHGATE_TRACE_LCMAIN=1 MACHGATE_TARBALL=/path/to/machgate-0.3.9-linux-arm64.tar.gz scripts/run-macho-docker.sh /path/to/macos-arm64-binary
```
