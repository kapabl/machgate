#!/bin/bash
# Test: machgate can parse NecroDancer arm64 fat binary
...
# machgate should be able to parse it (it will fail trying to run since deps aren't available,
...
# We just verify machgate starts parsing without errors.
output=$(timeout -s KILL 10 env MACHGATE_CONFIG=none "$BUILD_DIR/machgate" "$NECRO_BIN" 2>&1 || true)
